...
namespace SmartSchool.Controllers
{
    ...
    ?? throw new ArgumentNullException("Connection string 'DefaultConnection' not found.");
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<TeacherDTO>> GetTeacherByIdAsync(int id, [FromServices] IAuthorizationService authorizationService)
        {
            if (id < 1)
                return BadRequest("Invalid student id.");

            var Teacher = TeacherBll.GetTeacherById(id, _connectionString);
